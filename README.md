...
- `examples/tracing/kprobe`: Per-PID execve counter. Use `build.sh` to generate BPF object + skeleton.
- `examples/network/xdp`: Drop/pass example. Attach/detach with bpftool.
- `examples/tracing/fentry_fexit`: Record function entry with fentry, measure execution time with fexit.
- `examples/security/lsm`: File open filter (requires root, kernel ≥ 5.7 recommended).
