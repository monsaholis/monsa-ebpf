...
// S2-advanced socket_filter user-space controller.
...
static int handle_event(void *ctx, void *data, size_t data_sz) {
    (void)ctx;
    if (data_sz < sizeof(struct socket_filter_event))
        return 0;

    const struct socket_filter_event *evt = data;
    struct in_addr src, dst;
    src.s_addr = evt->saddr;
    dst.s_addr = evt->daddr;

    printf("=== Socket Filter Event (sampled) ===\n");
...
int main(int argc, char **argv) {
    ...
    err = example_socket_filter_bpf__attach(skel);
    if (err) {
        fprintf(stderr, "Failed to attach socket_filter\n");
        goto cleanup;
    }

    rb = ring_buffer__new(bpf_map__fd(skel->maps.sf_events), handle_event, NULL, NULL);
    if (!rb) {
        err = -1;
        goto cleanup;
    }

    printf("Socket Filter monitor started. Ctrl+C to stop.\n\n");
    while (!stop)
